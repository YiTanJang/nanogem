...
export async function run(_args: string[]): Promise<void> {
  ...
  const configuredRuntime = envContent.match(/^CONTAINER_RUNTIME=(.*)$/m)?.[1] || 'docker';

  if (configuredRuntime === 'k8s') {
    try {
      execSync('kubectl cluster-info', { stdio: 'ignore' });
      containerRuntime = 'k8s';
    } catch {
      containerRuntime = 'k8s (disconnected)';
    }
  } else {
    try {
  ...
  if (envContent && /^GEMINI_API_KEY=/m.test(envContent)) {
