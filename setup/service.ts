...
/**
 * Detect stale k8s config/token in the user systemd session.
 */
function checkK8sConfigStale(): boolean {
  try {
    execSync('systemd-run --user --pipe --wait kubectl cluster-info', {
      stdio: 'pipe',
      timeout: 10000,
    });
    return false; // k8s works from systemd session
  } catch {
    try {
      execSync('kubectl cluster-info', { stdio: 'pipe', timeout: 5000 });
      return true; // Works in shell but not systemd session → stale config
    } catch {
      return false; // k8s itself is not working
    }
  }
}
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  const k8sConfigStale = !runningAsRoot && checkK8sConfigStale();
  if (k8sConfigStale) {
    logger.warn(
      'Kubernetes config not active in systemd session — ~/.kube/config may have been updated mid-session',
    );
  }
  ...
  emitStatus('SETUP_SERVICE', {
    ...
    ...(k8sConfigStale ? { K8S_CONFIG_STALE: true } : {}),
    STATUS: 'success',
    LOG: 'logs/setup.log',
  });
}
