...
import { BASE_DIR, NANOGEM_DIR } from './constants.js';
...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, NANOGEM_DIR, 'state.yaml');
    ...
    try {
      ...
      // Save combined patch
      const patchPath = path.join(projectRoot, NANOGEM_DIR, 'combined.patch');
      ...
      if (newBasePath) {
        ...
        if (mergeConflicts.length > 0) {
          // Return with backup pending for Gemini CLI / user resolution
          return {
            success: false,
            patchFile: patchPath,
            filesInPatch,
