...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.nanogem', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.nanogem', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.nanogem', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.nanogem/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.nanogem', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.nanogem', 'base', 'src', 'config.ts'),
      'utf-8',
    );
